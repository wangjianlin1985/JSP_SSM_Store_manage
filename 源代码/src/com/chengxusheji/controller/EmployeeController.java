...
public class EmployeeController extends BaseController {
	...
	/*ajax方式按照查询条件分页查询雇员信息*/
	@RequestMapping(value = { "/list" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void list(String employeeNo,String name,String birthDate,String telephone,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<Employee> employeeList = employeeService.queryEmployee(employeeNo, name, birthDate, telephone, page);
	    /*计算总的页数和总的记录数*/
	    employeeService.queryTotalPageAndRecordNumber(employeeNo, name, birthDate, telephone);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String employeeNo,String name,String birthDate,String telephone,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (employeeNo == null) employeeNo = "";
		if (name == null) name = "";
		if (birthDate == null) birthDate = "";
		if (telephone == null) telephone = "";
		List<Employee> employeeList = employeeService.queryEmployee(employeeNo, name, birthDate, telephone, currentPage);
	    /*计算总的页数和总的记录数*/
	    employeeService.queryTotalPageAndRecordNumber(employeeNo, name, birthDate, telephone);
	    /*获取到总的页码数目*/
	    int totalPage = employeeService.getTotalPage();
	...
	public String delete(@PathVariable String employeeNo,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  employeeService.deleteEmployee(employeeNo);
	            request.setAttribute("message", "雇员删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "雇员删除失败!");
				return "error";
	...
	public void OutToExcel(String employeeNo,String name,String birthDate,String telephone, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(employeeNo == null) employeeNo = "";
        if(name == null) name = "";
        if(birthDate == null) birthDate = "";
        if(telephone == null) telephone = "";
        List<Employee> employeeList = employeeService.queryEmployee(employeeNo,name,birthDate,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Employee信息记录"; 
        String[] headers = { "雇员编号","姓名","性别","出生日期","雇员照片","联系电话","邮箱","注册时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<employeeList.size();i++) {
        	Employee employee = employeeList.get(i); 
        	dataset.add(new String[]{employee.getEmployeeNo(),employee.getName(),employee.getGender(),employee.getBirthDate(),employee.getEmployeePhoto(),employee.getTelephone(),employee.getEmail(),employee.getRegTime()});
        }
        /*
